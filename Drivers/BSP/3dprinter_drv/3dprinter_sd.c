...
uint8_t BSP_SD_Init(void) {
	...
	pSd->uSdHandle.Init.HardwareFlowControl =
...
//	if (HAL_SD_Init(&(pSd->uSdHandle), &(pSd->uSdCardInfo)) != SD_OK)
	if (HAL_SD_Init(&(pSd->uSdHandle)) != HAL_OK)
	{
		SD_state = MSD_ERROR;
	}
...
//		if (HAL_SD_WideBusOperation_Config(&(pSd->uSdHandle), SDIO_BUS_WIDE_4B)
//				!= SD_OK) {
//			SD_state = MSD_ERROR;
//		} else {
//			SD_state = MSD_OK;
...
uint8_t BSP_SD_ReadBlocks_DMA(uint32_t *pData, uint64_t ReadAddr, uint32_t BlockSize, uint32_t NumOfBlocks)
{
  ...
  if(HAL_SD_ReadBlocks_DMA(&(pSd->uSdHandle), pData, ReadAddr, NumOfBlocks) != HAL_OK)
  {
    SD_state = MSD_ERROR;
  }
  ...
  if(SD_state == MSD_OK)
  {
//    if(HAL_SD_CheckReadOperation(&(pSd->uSdHandle), (uint32_t)SD_DATATIMEOUT) != HAL_OK)
//    {
//      SD_state = MSD_ERROR;
//    }
//    else
...
uint8_t BSP_SD_WriteBlocks_DMA(uint32_t *pData, uint64_t WriteAddr, uint32_t BlockSize, uint32_t NumOfBlocks)
{
  ...
  if(HAL_SD_WriteBlocks_DMA(&(pSd->uSdHandle), pData, WriteAddr, NumOfBlocks) != HAL_OK)
  {
    SD_state = MSD_ERROR;
  }
  ...
  if(SD_state == MSD_OK)
  {
//    if(HAL_SD_CheckWriteOperation(&(pSd->uSdHandle), (uint32_t)SD_DATATIMEOUT) != HAL_OK)
//    {
//      SD_state = MSD_ERROR;
//    }
//    else
...
{
  BspSdDataType *pSd = &gBspSdData;
  if(HAL_SD_Erase(&(pSd->uSdHandle), StartAddr, EndAddr) != HAL_OK)
  {
    return MSD_ERROR;
  }
  else
